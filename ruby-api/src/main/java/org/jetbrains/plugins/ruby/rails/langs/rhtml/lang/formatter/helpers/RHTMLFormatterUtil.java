...
public class RHTMLFormatterUtil
{
	...
	static boolean checkIfInParentClosingTagEnd(@Nonnull final ASTNode parentNode, @Nullable final PsiElement htmlPsi)
	{
		if(htmlPsi != null)
		{
			...
			return (node != null && node.getElementType() == XmlElementType.XML_END_TAG_START && node.getTreeParent() == parentNode);
		}
		return false;
	}
	...
	public static RCompoundStatement getParentRCmpStByRHTMLOrHTMLChildNode(@Nonnull final FileViewProvider provider, @Nonnull final ASTNode node)
	{
		...
		return isRHTMLXmlTagForRubyBlockEnd(node, cmpSt, provider) ? RCompoundStatementNavigator.getParentCompoundStatement(cmpSt) : cmpSt;
	}
	...
	public static PsiElement findUpperHTMLElement(@Nonnull final FileViewProvider provider, final int startOffset, @Nullable final TextRange boundsTRange)
	{
		...
		while(!(htmlPsi instanceof XmlText || htmlPsi instanceof XmlDocument
				...
				//bounds condition
				&& (boundsTRange == null || (isRegisteredContainerElement(htmlPsiParent)) || containsTextRange(boundsTRange, htmlParentRange)))
		{
	 ...
	 */
	public static PsiElement findRHTMLElementByStartOffset(final FileViewProvider vProvider, final int startOffset, final boolean searchHighest)
	{
		...
		if(searchHighest && psiElement != null)
		{
			...
			while(psiParent != null && !(psiParent instanceof XmlDocument) && psiParent.getTextRange().getStartOffset() == startOffset)
			{
	...
	public static boolean isRHTMLXmlTagForRubyBlockEnd(@Nonnull final ASTNode rhtmlTagNode, @Nonnull RCompoundStatement nodeCmpSt, @Nonnull final FileViewProvider vProvider)
	{
		...
		if(cmpStEndInRHTML != null)
		{
			...
			final RHTMLRubyInjectionTag injectionTag = RHTMLRubyInjectionTagNavigator.getByPsiElement(cmpStEndInRHTML);
	...
	{
		return isTagOrAttrElementOrAttrValue(htmlPsi) || isSpecialElement(htmlPsi);
	}
	...
	{
		return htmlPsi instanceof XmlTag || htmlPsi instanceof XmlAttribute || htmlPsi instanceof XmlAttributeValue || htmlPsi instanceof XmlDoctype;
	}
