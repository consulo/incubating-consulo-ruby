...
import org.jetbrains.plugins.ruby.rails.actions.shortcuts.GeneratorNodeInfo;
import org.jetbrains.plugins.ruby.rails.actions.shortcuts.RakeTaskNodeInfo;
 ...
 * @date: Nov 16, 2006
 */
public class RSettings implements Configurable
{
	...
	private void registerDefaultActions(final Keymap keymap)
	{
		...
		keymap.addShortcut(GeneratorNodeInfo.getActionId("controller"), new KeyboardShortcut(ctrAltG, KeyStroke.getKeyStroke("C")));
		keymap.addShortcut(GeneratorNodeInfo.getActionId("model"), new KeyboardShortcut(ctrAltG, KeyStroke.getKeyStroke("M")));
		keymap.addShortcut(GeneratorNodeInfo.getActionId("migration"), new KeyboardShortcut(ctrAltG, KeyStroke.getKeyStroke("I")));
		keymap.addShortcut(GeneratorNodeInfo.getActionId("scaffold"), new KeyboardShortcut(ctrAltG, KeyStroke.getKeyStroke("S")));

		//RakeTasks
		final KeyStroke ctrAltR = KeyStroke.getKeyStroke("ctrl alt R");
		keymap.addShortcut(RakeTaskNodeInfo.getActionId("db:migrate"), new KeyboardShortcut(ctrAltR, KeyStroke.getKeyStroke("I")));
	}
