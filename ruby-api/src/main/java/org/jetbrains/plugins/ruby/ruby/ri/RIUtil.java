...
class RIUtil
{
	...
	private static void processMultipleRefinements(@Nonnull final StringBuilder sb, @Nonnull final String outStr)
	{
		...
		for(String item : items)
		{
			...
			if(item.length() > 0)
			{
				sb.append("    <a href='").append(item).append("'>").append(item).append("</a>\n");
			}
		}
	}
	...
	private static StringBuilder csvToLinkedMethodList(@Nonnull final StringBuilder sb, @Nonnull final String csvStr, @Nonnull final String lookupItem, final char riDelim)
	{
		...
		for(String method : methods)
		{
			...
			if(method.length() > 0)
			{
				sb.append("    <a href='").append(lookupItem).append(riDelim).append(method).append("'>").append(method).append("</a>\n");
			}
		}
		return sb;
	}
	 ...
	 * @return Output
	 */
	public static Output getRiOutput(final Sdk jdk, final Project project, final String progressTitle, final String... arguments)
	{
