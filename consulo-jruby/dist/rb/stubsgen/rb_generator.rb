...
module Generators
  class RBGenerator
    ...
    INDENT = "    "
    UNKNOWN_PARAMETERS = "(*smth) #paramteres couldn`t be extracted"
    ...
    def generate(toplevels)
        ...
        RDoc::TopLevel.all_classes_and_modules.each do |class_or_module|
            @loaded_files = Set.new
            @top_class_sym = class_or_module.name.downcase.to_sym
            @loaded_files.add @top_class_sym
            @file = File.new("#{@top_class_sym}.rb", File::CREAT|File::TRUNC|File::RDWR)
            @file << <<-DOC
=begin
 This is a machine generated stub using stdlib-doc for <b>#{(class_or_module.is_module? ? "module " : "class ") + class_or_module.name}</b>
 Ruby sources used:  #{$RUBY_SOURCE_VERSION}
...
=end
        ...
        puts @method_patched_params
        puts "\n\nIncorrect call_seqs(patched and not patched) [#{@method_incorrect_call_seq.size}]"
        puts @method_incorrect_call_seq
        puts "\n\n'*smth' params [#{@method_smth_params.size}]:"
        puts @method_smth_params
    end

    def require_if_needed someName, indent = 0
      someName = someName.gsub /::.+/, ""
      symbol = someName.downcase.to_sym
    ...
    def process_class class_or_module, indent=0
      ...
      # superclass require
      unless class_or_module.is_module?
        require_if_needed superclass, indent if superclass
      end

      # mixin requires if needed
      class_or_module.includes.map do |included|
        require_if_needed included.name, indent
      end
