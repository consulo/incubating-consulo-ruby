...
public class RHTMLBlock extends AbstractBlock
{
	...
	public RHTMLBlock(@NotNull final ASTNode node, @Nullable final Indent indent, @Nullable final Wrap wrap, final XmlFormattingPolicy xmlFormattingPolicy, @Nullable final TextRange textRange, @Nullable final Alignment alignment)
	{
		...
		final FileViewProvider provider = nodePsi == null ? null : nodePsi.getContainingFile().getViewProvider();
	...
	public ChildAttributes getChildAttributes(int newChildIndex)
	{
		...
		if(newChildIndex != 0 && newChildIndex < subBlocks.size())
		{
			...
			final ASTNode curNode = currBlock instanceof AbstractBlock ? ((AbstractBlock) currBlock).getNode() : null;
