...
public class RSpecComponentsInstallerTab extends TabbedSdkDependSettingsEditorTab
{
	...
	public RSpecComponentsInstallerTab(@NotNull final RailsWizardSettingsHolder settingsHolder, @Nullable final Project project)
	{
		...
		myLSvnMustBeInPath.setIcon(TargetAWT.to(RailsIcons.WARINIG_ICON));
	...
	public void beforeShow()
	{
		...
		if(myRSpecGemVersion == null)
		{
			...
			if(!gemExists)
			{
				myRBUseRSpecGem.setText(RBundle.message("module.settings.dialog.select.test.spec.rb.use.rspec.gem", RBundle.message("module.settings.dialog.select.test.spec.rb.use.rspec.gem.version.not.installed")));
			}
			else
			{
	...
	public static RailsWizardSettingsHolder.RSpecConfiguration getStoredDefaultConf()
	{
		...
		final boolean enableRSpecSupport = settings.wizardRailsFacetIsRSpecEnabled && !TextUtil.isEmpty(settings.wizardRailsFacetRSpecArgs);

		final boolean enableRSpecRailsSupport = settings.wizardRailsFacetIsRSpecRailsEnabled && !TextUtil.isEmpty(settings.wizardRailsFacetRSpecRailsArgs);
