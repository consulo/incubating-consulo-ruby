...
public class RailsUpdateHighlightersUtil
{
	...
	public static void setLineMarkersToEditor(final Project project, final Document document, final Collection<RailsLineMarkerInfo> markers, final int group)
	{
		...
		{
			RangeHighlighter marker = markupModel.addRangeHighlighter(info.startOffset, info.startOffset, HighlighterLayer.ADDITIONAL_SYNTAX, info.attributes, HighlighterTargetArea.EXACT_RANGE);
	...
	private static boolean isLineMarkerInGroup(final RailsLineMarkerInfo.MarkerType type, final int group)
	{
		switch(type)
		{
