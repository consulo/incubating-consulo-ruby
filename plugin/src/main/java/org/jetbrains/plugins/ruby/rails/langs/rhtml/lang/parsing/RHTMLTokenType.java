...
public interface RHTMLTokenType extends XmlTokenType
{
	...
	//Helpers
	// All comments tokens
	public TokenSet RHTML_ALL_COMMENT_TOKENS = TokenSet.create(RHTML_COMMENT_START, RHTML_COMMENT_CHARACTERS, RHTML_COMMENT_END, RHTML_COMMENT_EOL, RHTML_COMMENT_WSL);

	// Start separators for non comment injections
	public TokenSet RHTML_SEPARATORS_STARTS = TokenSet.create(RHTML_EXPRESSION_START, RHTML_SCRIPTLET_START);
	// Ends separators for non comment injections
	public TokenSet RHTML_SEPARATORS_ENDS = TokenSet.create(RHTML_EXPRESSION_END, RHTML_SCRIPTLET_END);

	// Ends separators for non comment injections
	public TokenSet RHTML_ALL_SEPARATORS_ENDS = TokenSet.create(RHTML_EXPRESSION_END, RHTML_SCRIPTLET_END, RHTML_COMMENT_END);

	// Separators(start/end) for rhtml scriplets
	TokenSet RHTML_SEPARATORS = TokenSet.create(RHTML_SCRIPTLET_START, RHTML_SCRIPTLET_END,

			RHTML_EXPRESSION_START, RHTML_EXPRESSION_END,

			RHTML_COMMENT_START, RHTML_COMMENT_END,

			FLEX_ERROR);

	// Start/end elements for brace matcher
	TokenSet RHTML_ALL_INJECTIONS_START_END_TOKENS = TokenSet.create(RHTML_SCRIPTLET_START, RHTML_SCRIPTLET_END,

			RHTML_EXPRESSION_START, RHTML_EXPRESSION_END,

			RHTML_COMMENT_START, RHTML_COMMENT_END);

	TokenSet RHTML_WHITE_SPECE_TOKENS = TokenSet.create(RHTML_COMMENT_EOL, RHTML_COMMENT_WSL);
}
