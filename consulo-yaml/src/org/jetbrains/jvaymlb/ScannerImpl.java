...
public class ScannerImpl implements Scanner
{
	...
	private boolean isEnding()
	{
		...
		return (this.buffer.bytes[this.pointer] & 0xFF) == '-' &&
				...
				!(this.buffer.realSize <= (this.pointer + 4) || ((this.buffer.bytes[this.pointer + 3] == '\n') && (this.buffer.bytes[this.pointer + 4] == 0))) &&
	...
	private boolean isEndOrStart()
	{
		...
		return (((this.buffer.bytes[this.pointer] & 0xFF) == '-' &&
				(this.buffer.bytes[this.pointer + 1] & 0xFF) == '-' &&
				(this.buffer.bytes[this.pointer + 2] & 0xFF) == '-') || ((this.buffer.bytes[this.pointer] & 0xFF) == '.' &&
				(this.buffer.bytes[this.pointer + 1] & 0xFF) == '.' &&
				(this.buffer.bytes[this.pointer + 2] & 0xFF) == '.')) && (NULL_BL_T_LINEBR[this.buffer.bytes[this.pointer + 3]]);
	}
	...
	private Token fetchMoreTokens()
	{
		...
		switch(ch)
		{
			...
			case '!':
				return fetchTag();
			case '|':
				if(this.flowLevel == 0 && CHOMPING[peek(1)])
				{
					return fetchLiteral();
				}
				break;
		...
		if(STUPID_CHAR[this.buffer.bytes[this.pointer] & 0xFF] || (ensure(1, false) && (this.buffer.bytes[this.pointer] == '-' ||
	...
	private Token scanTag()
	{
		...
		return new TagToken(new ByteList[]{
				handle,
				suffix
		}, last_offset, offset);
	}
