...
public class RTypeUtil {
    ...
    public static RType createTypeBySymbol(@Nullable final FileSymbol fileSymbol,
     ...
     */
    private static RType createTypeByUsage(final FileSymbol fileSymbol,
                                           @NotNull final RPsiElement element) {
      final TypeInferenceHelper helper = RubyPsiManager.getInstance(element.getProject()).getTypeInferenceHelper();
     ...
     */
    public static DuckType createDuckTypeBySymbol(@Nullable final FileSymbol fileSymbol,
...
// Adding JRuby Specific children
        ...
        return createDuckTypeByChildren(fileSymbol, children,
                new HashSet<Symbol>(symbol.getChildren(fileSymbol).getAll()),
                createFilter(inReference));
    }


    public static SymbolFilter createFilter(final boolean inReference) {
        return SymbolFilterFactory.createFilterByTypeSet(inReference ?
                Types.REFERENCE_AUTOCOMPLETE_TYPES :
                Types.EMPTY_CONTEXT_AUTOCOMPLETE_TYPES);
    }

    public static DuckType createDuckTypeByChildren(@Nullable final FileSymbol fileSymbol,
                                                     @NotNull final Children children,
                                                     @NotNull final Set directChildren,
                                                     @NotNull final SymbolFilter filter) {
...
// Here we process all others
        for (Symbol child : children.getChildrenByFilter(filter).getAll()) {
            ...
            if (!Types.FIELDS.contains(child.getType()) &&
                    !Type.JAVA_FIELD.asSet().contains(child.getType())) {
...
// Add others
    ...
    @NotNull
    private static Message createMessageBySymbol(@Nullable final FileSymbol fileSymbol,
                                                 @NotNull final Symbol symbol,
    ...
    public static List<RubyLookupItem> getLookupItemsByType(@NotNull final RType type,
                                                            @Nullable final String name,
                                                            @Nullable final SymbolFilter filter) {
    ...
    public static Collection<Message> intersection(@NotNull final Collection<Message> collection1,
                                                  @NotNull final Collection<Message> collection2){
...
// Here we just iterate over collections to construct name to Message maps
    ...
    public static Collection<Message> union(@NotNull final Collection<Message> collection1,
                                            @NotNull final Collection<Message> collection2){
        if (collection1.isEmpty()){
            return collection2;
        }
