...
public class SymbolPresentationUtil {
    ...
    @Nullable
    public static RubyLookupItem createRubyLookupItem(@NotNull final Symbol symbol,
                                                      @NotNull String name,
                                                      boolean bold,
                                                      final boolean multiMessage) {
...
// Setting icon for lookupItem
        ...
        if (lastPrototype instanceof RVirtualConstant) {
        ...
        if (lastPrototype instanceof RVirtualAlias){
            icon = ((RVirtualAlias) lastPrototype).getIcon(Iconable.ICON_FLAG_OPEN);
        } else
        if (lastPrototype instanceof RVirtualFieldAttr){
            icon = RFieldAttrPresentationUtil.getAttrIcon(((RVirtualFieldAttr) lastPrototype).getFieldAttrType());
        } else
        if (lastPrototype instanceof RVirtualImportJavaClass){
            bold = true;
            icon = JavaClassPackagePresentationUtil.getIncludeIcon();
        } else
        if (lastPrototype instanceof RVirtualIncludeJavaClass){
            bold = true;
            icon = JavaClassPackagePresentationUtil.getIncludeIcon();
        } else
        ...
        return lastPrototype != null ?
                new RubyPsiLookupItem(project, name, tailText, typeText, lastPrototype, priority, bold, icon) :
                new RubySimpleLookupItem(name, typeText, priority, bold, icon);
    }
    ...
    public static String getPresentableNameWithLocation(@Nullable final FileSymbol fileSymbol, @NotNull final Symbol symbol){
        ...
        return parent.getType() != Type.FILE ?
                name + " " + RBundle.message("in") + " " +  SymbolUtil.getPresentablePath(parent) :
                name;
    }


    @NotNull
    public static List<RVirtualElement> getPrototypesToShow(@Nullable final FileSymbol fileSymbol,
