...
class MatchingExpression implements RubyTokenTypes {
    ...
    private static IElementType parseHighPriorityWithLeadBitExpr(final RBuilder builder, final RMarker marker, final IElementType result){
        ...
        return BinaryExprParsing.parseWithLeadOperand(builder,
                ...
                parsingMethod,
                ErrorMsg.EXPRESSION_EXPECTED_MESSAGE,
                BNF.tMATCH_HIGH_PRIORITY_OPS,
                RubyElementTypes.BOOL_MATCHING_EXPRESSION);

    }

    public static IElementType parseWithLeadPRIMARY(final RBuilder builder, final RMarker marker, final IElementType result) {
        return parseLowPriorityWithLeadHigh(builder, marker.precede(),
                parseHighPriorityWithLeadBitExpr(builder, marker.precede(),
                        BitExpression.parseWithLeadPRIMARY(builder, marker, result)));
    }
}
