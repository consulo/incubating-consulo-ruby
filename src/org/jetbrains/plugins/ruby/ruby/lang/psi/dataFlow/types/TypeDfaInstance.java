        ...
        if (instruction instanceof ReadWriteInstruction) {
            ...
            if (access instanceof MethodParameterAccess){
                ...
                if (argType == ArgumentInfo.Type.ARRAY) {
                    final RType type = RTypeUtil.createTypeBySymbol(myFileSymbol,
                            SymbolUtil.getTopLevelClassByName(myFileSymbol, CoreTypes.Array), Context.INSTANCE, true);
                    localVariablesMap.put(id, type);
                }
                if (argType == ArgumentInfo.Type.BLOCK) {
                    final RType type = RTypeUtil.createTypeBySymbol(myFileSymbol,
                            SymbolUtil.getTopLevelClassByName(myFileSymbol, CoreTypes.Proc), Context.INSTANCE, true);
                    localVariablesMap.put(id, type);
                }
...
// if such a field writer can be resolved, we don`t add it manually
                    if (oldType.getMessagesForName(text).isEmpty()) {
                        final RType type = oldType.addMessage(new MessageImpl(text, 0,
                                true, new CallSymbol(text, Type.FIELD_WRITE_ACCESS, ref)));
                        map.put(name, type);
                    }
                }
            }
...
// if such a constant can be resolved, we don`t add it manually
                    if (oldType.getMessagesForName(text).isEmpty()) {
                        final RType type = oldType.addMessage(new MessageImpl(text, 0,
                                true, new CallSymbol(text, Type.CONSTANT_ACCESS, ref)));
                        map.put(name, type);
                    }
                }
            }
...
// if such a method call can be resolved, we don`t add it manually
                    if (oldType.getMessagesForName(text).isEmpty()) {
                        final RType type = oldType.addMessage(new MessageImpl(text, callAccess.getNumberOfArgs(),
                                true, new CallSymbol(text, Type.CALL_ACCESS, ref)));
                        map.put(name, type);
                    }
                }
            }
