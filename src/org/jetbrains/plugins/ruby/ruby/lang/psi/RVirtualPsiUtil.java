...
public class RVirtualPsiUtil {
    ...
    public static List<RubyFilesCache> getCaches(@NotNull final Project project,
        ...
        final RubyModuleCachesManager manager =
                (module != null) ? RCacheUtil.getCachesManager(module) : null;
     ...
     */
    @Nullable
    public static RPsiElement findPsiByVirtualElement(@NotNull final RVirtualElement element,
    ...
    @Nullable
    private static RVirtualContainer findVirtualContainer(@NotNull final RContainer container,
    ...
    @Nullable
    public static RConstant findRConstant(@NotNull final RVirtualConstant constant,
                                          @NotNull final Project project){
    ...
    @Nullable
    public static RField findRField(@NotNull final RVirtualField field,
                                    @NotNull final Project project){
    ...
    @Nullable
    public static RGlobalVariable findRGlobalVar(@NotNull final RVirtualGlobalVar var,
                                                 @NotNull final Project project){
    ...
    @Nullable
    private static RVirtualStructuralElement findByPath(@NotNull final RVirtualContainer root,
    ...
    @Nullable
    public static RVirtualMethod getMethodWithoutArgumentsByName(@NotNull final RVirtualContainer container,
                                                      @Nullable final String name) {
        for (RVirtualStructuralElement element :
                RContainerUtil.selectVirtualElementsByType(container.getVirtualStructureElements(), StructureType.METHOD)) {
    ...
    public static boolean areMethodsEqual(@NotNull final RVirtualMethod method, @NotNull final RVirtualMethod otherMethod) {
        ...
        if (!method.getName().equals(otherMethod.getName())
            || method.getAccessModifier() != otherMethod.getAccessModifier() ||
                !method.getArgumentInfos().equals(otherMethod.getArgumentInfos())) {
            return false;
        }
        final List<RVirtualStructuralElement> subCandidateMethods =
                RContainerUtil.selectVirtualElementsByType(otherMethod.getVirtualStructureElements(), StructureType.METHOD);
        final List<RVirtualStructuralElement> subMethods =
                RContainerUtil.selectVirtualElementsByType(method.getVirtualStructureElements(), StructureType.METHOD);
...
// Getting caches
        RubyFilesCache cache = getCacheForFile(url, RCacheUtil.getCachesByFile(file, SearchScopeUtil.getAllSearchScope(), project));
        return cache!=null ? cache.getUp2DateFileInfo(file) : null;
    }

    public static boolean areSubStructureEqual(@NotNull final RContainer container,
                                               @NotNull final RVirtualContainer vContainer){
// Substructure check
...
//                System.err.println(structuralElement + " is not equal to " + virtualElement);
    ...
    public static boolean areConstantHoldersEqual(@NotNull final RVirtualConstantHolder holder1,
                                                  @NotNull final RVirtualConstantHolder holder2){
    ...
    @SuppressWarnings({"BooleanMethodIsAlwaysInverted"})
    public static boolean areGlobalVariableHoldersEqual(@NotNull final RVirtualGlobalVarHolder holder1,
                                                        @NotNull final RVirtualGlobalVarHolder holder2){
    ...
    @SuppressWarnings({"BooleanMethodIsAlwaysInverted"})
    public static boolean areFieldHoldersEqual(@NotNull final RVirtualFieldHolder holder1,
                                               @NotNull final RVirtualFieldHolder holder2){
