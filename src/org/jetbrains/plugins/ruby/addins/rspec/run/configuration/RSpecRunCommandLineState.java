...
public class RSpecRunCommandLineState extends AbstractRTestsCommandLineState {
    ...
    public GeneralCommandLine createCommandLine() throws ExecutionException {
        ...
        if (myConfig.shouldUseCustomSpecRunner()) {
            ...
            rspecScriptPath = myConfig.getCustomSpecsRunnerPath();
                ...
                && !myConfig.shouldUseAlternativeSdk()
                && RailsFacetUtil.hasRailsSupport(module)
                && RSpecModuleSettings.getInstance(module).getRSpecSupportType() == RSpecModuleSettings.RSpecSupportType.RAILS_PLUGIN) {
            //rais rspec
    ...
    private String pathcSpecArgsWithColouringArgIfNecessary(@NotNull final String args) {
        if (myConfig.shouldUseColoredOutput()) {
            if (!args.startsWith(RSpecUtil.COLOURED_COMMAND_LINE_ARG)
                && !args.contains(" " + RSpecUtil.COLOURED_COMMAND_LINE_ARG)) {
    ...
    private void createRunInFolderConfScript(@Nullable final String rspecScriptPath,
                                             final String rubyExecutablePath,
                                             final boolean runSeparately) throws IOException {
