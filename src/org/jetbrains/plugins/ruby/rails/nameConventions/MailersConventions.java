...
public class MailersConventions {
    ...
    public static boolean isMailerFile(@NotNull final RVirtualFile rFile,
    ...
    public static boolean isMailerClass(@Nullable final RVirtualClass rClass,
        ...
        */
        ...
        return (path.size() == 2
                && MailersConventions.ACTION_MAILER_MODULE.equals(path.get(0)))
                && MailersConventions.BASE_CLASS.equals(path.get(1));
    }

     public static void loadBuiltInGems(@Nullable final FileSymbol fileSymbol,
        ...
        for (String activeSupportFileUrl : RFileUtil.findUrlsForName(fileSymbol, "action_mailer")) {
            ...
            final String actionMailerVendorDirUrl =
                    VirtualFileUtil.removeExtension(activeSupportFileUrl)
                            + VirtualFileUtil.VFS_PATH_SEPARATOR
                            + ACTION_MAILER_BUILT_IN_GEMS_VENDOR_NAME;
            final VirtualFile actionMallerVendorDir =
                    VirtualFileManager.getInstance().findFileByUrl(actionMailerVendorDirUrl);
            ...
            if (actionMallerVendorDir != null) {
                ...
                for (VirtualFile file : actionMallerVendorDir.getChildren()) {
                    if (file.isDirectory() && file.getName().startsWith(ACTION_MAILER_BUILT_IN_GEMS_TMAIL)) {
                        ...
                        if (tMailDir == null
                                || file.getName().compareTo(tMailDir.getName()) > 0) {
                            tMailDir = file;
                        }
                    }
                }
                if (tMailDir != null) {
                    ...
                    final String tmailUrl = tMailDirUrl + VirtualFileUtil.VFS_PATH_SEPARATOR
                                            + ACTION_MAILER_BUILT_IN_GEMS_TMAIL + "." + RubyFileType.RUBY.getDefaultExtension(); 
