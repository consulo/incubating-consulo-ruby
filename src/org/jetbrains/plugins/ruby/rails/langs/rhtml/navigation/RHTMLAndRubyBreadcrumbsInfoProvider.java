...
public class RHTMLAndRubyBreadcrumbsInfoProvider extends BreadcrumbsInfoProvider implements ProjectComponent {
    ...
    public PsiElement getParent(@NotNull final PsiElement psiElement) {
        ...
        //Parent is HTML
            ...
            while (parent != null && parent.getNode() != null
                   && !RHTMLTokenType.RHTML_SEPARATORS_STARTS.contains(parent.getNode().getElementType())) {
    ...
    private String getInjectionPresentation(final PsiElement start,
                                            final PsiElement startOmit,
                                            final PsiElement end,
                                            final PsiElement endOmit) {
    ...
    private String getPresentation(@NotNull final PsiElement psiElement, final boolean notForToolTip) {
        if (isInRubyDebugMode(psiElement)) {
            return notForToolTip
                    ? psiElement.toString()
                    : RBundle.message("breadcrumbs.rhtml.tooltip.textrange", psiElement.getTextRange());
        }
        ...
        if (lang == RubyLanguage.RUBY) {
            ...
            if (node != null) {
                ...
                if (RHTMLTokenType.RHTML_SEPARATORS_STARTS.contains(type)
                        || RHTMLTokenType.RHTML_SEPARATORS_ENDS.contains(type)) {
                    if (notForToolTip) {
                    ...
                    return notForToolTip
                        ? RBundle.message("breadcrumbs.rhtml.presentation.comment")
                        : RHTMLFoldingBuilder.RHTML_COMMENT_FOLD_TEXT;
                }
            }
        }
