...
public class RHTMLSpacingProcessor {
     ...
     */
    @Nullable
    public static Spacing getSpacing(@NotNull final Block parentBlock,
                                     @NotNull final ASTNode parentNode,
                                     @NotNull final ASTNode leftBlockNode,
                                     @NotNull final ASTNode rightBlockNode,
...
//injections
        if (parentNodeType == RHTMLElementType.RHTML_XML_TAG) {
            ...
            if (RHTMLTokenType.RHTML_SEPARATORS_STARTS.contains(leftBlockType)) {
                if (leftBlockType == RHTMLTokenType.RHTML_SCRIPTLET_START
                        && rightBlockType == RHTMLTokenType.OMIT_NEW_LINE) {
...
//attributes
        if (parentNodeType == XmlElementType.XML_ATTRIBUTE) {
            if (leftBlockType == XmlTokenType.XML_NAME
                    || leftBlockType == XmlTokenType.XML_EQ
                    || rightBlockType == XmlTokenType.XML_EQ
                    || rightBlockType == XmlTokenType.XML_ATTRIBUTE_VALUE_START_DELIMITER) {
