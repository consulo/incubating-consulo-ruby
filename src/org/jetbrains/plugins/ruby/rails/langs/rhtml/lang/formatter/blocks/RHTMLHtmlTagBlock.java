...
public class RHTMLHtmlTagBlock extends XmlTagBlock {
    ...
    public RHTMLHtmlTagBlock(ASTNode node, Wrap wrap, Alignment alignment,
    ...
    @NotNull
    protected NodeInfo processChildAndGetNext(final List<Block> result,
                                              final ASTNode htmlChild, final NodeInfo childNodeInfo,
                                              final Wrap wrap, final Alignment alignment, final Indent rhtmlIndent) {
        ...
        if (language == RubyLanguage.INSTANCE
            ...
            if (childNodeInfo.isTemplate()) {
                ...
                return RHTMLBlockGenerator.processContentChildNode(childNode, childNodeInfo, myNode, result,
                        myNodeCmpSt, myXmlFormattingPolicy, myViewProvider,
                        getTextRange(), alignment, rhtmlIndent);
            }
        ...
        while (child != null) {
            ...
            if (!FormatterUtil.containsWhiteSpacesOnly(child) && child.getTextLength() > 0) {
                ...
                if (childType == XmlElementType.XML_TAG_END) {
                    // ..>
                    childNodeInfo = processChildAndGetNext(localResult, child, childNodeInfo, wrap, alignment,
                                                            calcTagContentIndent(insideTag, childRCmpSt));
                    ...
                    childNodeInfo = processChildAndGetNext(localResult, child, childNodeInfo,
                                                           wrap, alignment,
                                                           calcTagContentIndent(insideTag, childRCmpSt));
                }
            } else {
    ...
    protected Indent getTagContentChildIndent(final int newChildIndex) {
        ...
        if (newChildIndex != 0 && newChildIndex < subBlocks.size()) {
            ...
            if (block instanceof SyntheticBlock) {
                ...
                if (curFirstSubBlock instanceof AbstractBlock) {
                    ...
                    if (newChildIndex == subBlocks.size() - 1
                            && curFirstSubBlockType == XmlTokenType.XML_END_TAG_START) {
                        ...
                        final RCompoundStatement firstSubBlockCmpst =
                                RHTMLFormatterUtil.getRCmpStNodeStartOffset(myViewProvider, curFirstSubBlockNode);
                        if (firstSubBlockCmpst != null) {
                            if (RHTMLFormatterUtil.isRHTMLXmlTagForRubyBlockEnd(curFirstSubBlockNode, firstSubBlockCmpst, myViewProvider)) {
                                ...
                                final RCompoundStatement parentCmpSt =
                                        ((RHTMLRubyInjectionBlock) curFirstSubBlock).getParentRCmpSt();
                                final ASTNode parentNode =
                                        ((RHTMLRubyInjectionBlock) curFirstSubBlock).getParentNode();
            ...
            final ASTNode node = block instanceof AbstractBlock
                    ? ((AbstractBlock) block).getNode()
                    : null;

            if (node != null) {
                final RCompoundStatement childCmpSt =
                        RHTMLFormatterUtil.getParentRCmpStByRHTMLOrHTMLChildNode(myViewProvider, node);
...
//        }
    ...
    private Alignment chooseAlignment(final ASTNode child,
                                      final Alignment attrAlignment, final Alignment textAlignment,
