...
public class SwitchToView extends EditorAction {
    ...
    protected static class Handler extends EditorActionHandler {
        public void execute(Editor editor, DataContext dataContext) {
            ...
            if (file == null || !RubyVirtualFileScanner.isRubyFile(file)
                    || file.getParent() == null) {
                cantNavigate();
                return;
            }
    ...
    public static void switchToView(final Module module, final RContainer container,
        ...
        if (container instanceof RMethod) {
            ...
            final List<VirtualFile> fileList =
                    ViewsConventions.getViews(method, controllerDirUrl,
                            controllerName,
                            module);
            final PsiFile[] psiFiles = getPsiFiles(module, fileList);
            RailsLineMarkerNavigator.openTargets(psiFiles,
                RBundle.message("codeInsight.rails.navigation.title.action.view", method.getName()),
                    new GotoFileCellRenderer(maxSize), relativePoint);
            ...
            final List<VirtualFile> fileList =
                    ViewsConventions.getPartialViews(controllerDirUrl, controllerName, module);

            final PsiFile[] psiFiles = getPsiFiles(module, fileList);
            RailsLineMarkerNavigator.openTargets(psiFiles,
                    RBundle.message("codeInsight.rails.navigation.title.action.partial.view", rClass.getName()),
                    new GotoFileCellRenderer(maxSize), relativePoint);
        }
    }
    ...
    public static boolean isSwitchToViewEnabled(@Nullable final RContainer methodOrClass,
        ...
        if (methodOrClass instanceof RMethod) {
            ...
            return viewsFolder != null
                   && ViewsConventions.containsViewsByViewsFolder(viewsFolder, name);

        } else if (methodOrClass instanceof RClass) {
            final VirtualFile viewsFolder =
                    ViewsConventions.getViewsFolderByClass(module, (RClass) methodOrClass);
            return viewsFolder != null
                   && !ViewsConventions.findPartialViews(viewsFolder).isEmpty();
        }
        return false;
    }
}
