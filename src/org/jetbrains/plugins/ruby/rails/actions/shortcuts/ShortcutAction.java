...
public class ShortcutAction extends AnAction {
    ...
    public ShortcutAction(@NotNull final String name,
                          @NotNull final String cmd,
                          final Icon icon,
                          final ShortcutsTreeState state) {
    ...
    public void actionPerformed(final AnActionEvent e) {
        ...
        switch (myState) {
            case GENERATORS_SUBTREE:
                ...
                msg = RBundle.message("action.registered.shortcut.execute.disabled.generators.msg",
                                      myCmdName, module.getName());
                break;
            case RAKE_SUBTREE:
                final RakeTask rootRakeTask = facetConfiguration.getRakeTasks();
                final RakeTask task = rootRakeTask != null
                                        ? RakeUtil.findTaksByFullCmd(rootRakeTask, myCmdName)
                                        : null;
                ...
                msg = RBundle.message("action.registered.shortcut.execute.disabled.raketasks.msg",
                        myCmdName, module.getName());
                break;
        }
        if (msg != null) {
    ...
    private boolean executeGenerateAction(final String[] generators, final AnActionEvent e) {
        ...
        for (String generator : generators) {
            ...
            if (myCmdName.equals(generator)) {
                final SimpleGeneratorAction action =
                        GeneratorsActionGroup.createGeneratorAction(name2Action, myCmdName);
