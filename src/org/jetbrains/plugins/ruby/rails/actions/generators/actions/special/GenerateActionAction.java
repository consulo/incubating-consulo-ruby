...
public class  GenerateActionAction extends SimpleGeneratorAction {
    protected static class ActionStubInserter implements Runnable {
        ...
        public ActionStubInserter(@NotNull final DataContext dataContext,
									...
									public void run() throws Exception {
                                        ...
                                        if (newMethod != null) {
                                            ...
                                            try {
                                                ...
                                                CodeStyleManager.getInstance(project).adjustLineIndent(document,
                                                        editor.getCaretModel().getOffset());
                                            } catch (IncorrectOperationException e) {
                                                LOG.error("Inserting method template for : " + myMethodName
...
*/
    ...
    public GenerateActionAction(final String actionName) {
        this(actionName != null ? actionName : RBundle.message("popup.generate.action.text"),
             RBundle.message("popup.generate.action.description"),
             RailsIcons.RAILS_ACTION_NODE);
    }

    public GenerateActionAction(@NotNull final String actionName,
                                @Nullable final String description,
                                @Nullable final Icon icon) {
        super(GenerateControllerAction.GENERATOR_CONTROLLER,
    ...
    public void invokeAction(final String scriptArguments,
                             ...
                             final DataContext myDataContext) {
        insertMethodStub(myDataContext, mainArgument);
        invokeAction(scriptArguments, mainArgument,  myModule);
    }

    public void insertMethodStub(final DataContext dataContext,
                                 final String methodName) {
        if (dataContext != null) {
            Project project = DataKeys.PROJECT.getData(dataContext);
               CommandProcessor.getInstance().executeCommand(project,
                       RModuleUtil.createWriteAction(new ActionStubInserter(dataContext,
                                                                            methodName)),
                       "GenerateActionAction.insertMethodSub", null);
            }
    }
	...
	public void update(@NotNull final AnActionEvent event) {
        ...
        if (rClass != null && file != null) {
            ...
            if (document != null) {
                ...
                final String controllerFullClassName =
                        ControllersConventions.getControllerClassName(ControllersConventions.getControllerFullName(module, file));
    ...
    protected ActionInputValidator createValidator(@NotNull final Module module,
    ...
    private static RClass determineControllerClass(final DataContext dataContext) {
        ...
        if (psiElem != null && psiElem instanceof PsiWhiteSpace
            && psiElem.isWritable()) {

            final RCompoundStatement st =
                    RCompoundStatementNavigator.getByPsiElement(psiElem);
            if (st != null) {
    ...
    private void invokeDialog(final Module module,
                                      @NotNull final DataContext dataContext) {
        final VirtualFile file = DataKeys.VIRTUAL_FILE.getData(dataContext);

        final ActionInputValidator validator = createValidator(module, file, dataContext);
        GenerateDialogs.showGenerateActionDialog(module,
