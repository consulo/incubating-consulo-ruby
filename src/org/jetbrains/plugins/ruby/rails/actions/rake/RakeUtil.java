...
public class RakeUtil
{
    ...
    */
	...
	@Nullable
	public static Output getRakeGemAvailableTasksOutput(final Sdk sdk, final Project project, final String errorTitle, final String contentRoot)
	{
	...
	public static void runRakeTask(@NotNull final DataContext dataContext, @NotNull final RakeTask task)
	{
		...
		if(sdk != null)
		{
			GemsRunner.runGemScriptInConsoleAndRefreshModule(module, sdk, title, provider.getActions(), true, RAKE, workingDir, provider, null, filters, null);
			RailsFacetUtil.refreshRailsAppHomeContent(module);
		}
		else
		{
			final String msg = RBundle.message("action.registered.shortcut.execute.disabled.raketasks.msg", task.getFullCommand(), module.getName()) + " " + RBundle.message("sdk.no.specified");
			Messages.showErrorDialog(module.getProject(), msg, RBundle.message("action.registered.shortcut.execute.disabled.title"));
		}
	}
