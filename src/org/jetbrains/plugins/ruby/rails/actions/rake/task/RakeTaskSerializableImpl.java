...
public class RakeTaskSerializableImpl implements RakeTask, Serializable {
    ...
    public RakeTaskSerializableImpl(final String id, final String description,
                                    final String fullCmd, final boolean group,
                                    @Nullable final RakeTaskSerializableImpl parent) {
        this.id = id;
    ...
    public void registerNewCommand(final RakeCommand command){
        ...
        for (int i = 0; i < ids.length;i++) {
            ...
            RakeTaskSerializableImpl task =
                    (RakeTaskSerializableImpl) RakeUtil.findSubTaskById(ids[i], isGroup, parent);
            if (task == null) {
                task = new RakeTaskSerializableImpl(ids[i],
                                                isGroup ? null: command.getDescription(),
                                                null, isGroup, parent);
