...
public class BaseRailsFacetBuilder {
    ...
    public static void initFacetInstance(final BaseRailsFacet railsFacet) {
        ...
        if (!conf.isInitialized()) {
            if (uncommitedModule.isLoaded()) {
                //When facet has been created
                //  * from [Settings]->[Project Settings]->[Module]
                //  * by OnTheFly facet detector

                // Settings Tab is created before sending facetAddedEvent ot BaseRailsFacetListener!
                ...
                if (conf.getNullableRailsApplicationRootPath() != null) {
                    ...
                    final Runnable runnable = new Runnable() {
                        public void run() {
                            ...
                            configureRailsSourcePathes(RailsViewFoldersManager.getInstance(uncommitedModule),
                                                       railsFacet.getConfiguration().getRailsApplicationRootPathUrl(),
                    ...
                    conf.setInitialized();
                    StartupManager.getInstance(uncommitedModule.getProject()).runWhenProjectIsInitialized(runnable);
                }
                //else it has been created by new facet wizard
            }
        } else {
...
///////////////// Facet Configuration //////////////////////////////////////////////////////////
     ...
     */
    public static void setupGreenhornFacet(final ModifiableRootModel rootModel,
...
///////////////// Module Settings /////////////////////////
        ...
        if (!settings.isRSpecSupportEnabled()) {
            ...
            rSpecType = rSpecConfiguration.shouldInstallRSpecPlugin()
                         ? RSpecModuleSettings.RSpecSupportType.RAILS_PLUGIN : RSpecModuleSettings.RSpecSupportType.GEM;
        ...
        if (settings.getAppGenerateWay() == RailsWizardSettingsHolder.Generate.NEW) {
            ...
            if (appHomeDir != null && appHomeDir.isDirectory() &&
              ...
              final int dialogResult = Messages.showYesNoDialog(module.getProject(),
                                                                RBundle.message("module.rails.generateapp.rails.new.overwrite.message", applicationHomePath),
...
//////////////// Additional Sources, Rails View Custom folders
                ...
                if (rSpecConf.enableRSpecSupport() || rSpecConf.enableRSpecRailsSupport()) {
                    final String[] args = {GeneratorsUtil.GENERATE_SCRIPT, RSpecUtil.SPEC_GENERATOR_NAME};
    ...
    private static void installComponents(@NotNull final Module uncommitedModule,
        ...
        final Runnable runnable = new Runnable() {
            public void run() {
                ...
                generateRailsApplication(uncommitedModule, sdk, descFactory, settings, applicationHomePath,
                                         createOnRailsAppGenerated(descFactory, uncommitedModule, sdk,
                                                                    railsFacet, settings,
                                                                    applicationHomePath, project, moduleName));
             }
        };
    ...
    private static void installRSpecSupport(final Module module,
            ...
            public void run() {
                if (rSpecConf.enableRSpecRailsSupport() && rSpecConf.shouldInstallRSpecRailsPlugin()) {
                    PluginsUtil.installPluginAndUpdateModuleContent(
                            sdk,
                            module,
                            rSpecConf.getRSpecRailsArgs(),
                            descFactory, nextAction);
                }
            }
        };
        ...
        if (rSpecConf.enableRSpecSupport() && rSpecConf.shouldInstallRSpecPlugin()) {
            PluginsUtil.installPluginAndUpdateModuleContent(
                    sdk,
                    module,
                    rSpecConf.getRSpecArgs(),
                    descFactory, installRSpecRails);
        } else {
    ...
    private static void showError(final Exception e,
                                  final Project project, final String moduleName) {
    ...
    private static void configureRakeTasksAndGenerators(final Facet facet,
        ...
        if (module.isLoaded())  {
            ...
            final Runnable runnable = loadRakeTasksAndGeneratorsIfArentLoadedRunnalbe(sdk, facet);
    ...
    @Nullable
    private static String searchRailsAppInUnderDirectory(@NotNull final Module module,
     ...
     private static void configureRailsSourcePathes(final RailsViewFoldersManager rootManager,
                                                    @NotNull final String railsAppHomeRootUrl,
                                                    final boolean isRSpecSupportEnabled,
                                                    final Module module) {
    ...
    private static void addAdditionalRailsViewFolders(@NotNull final String railsAppHomeRootUrl,
                                                      @NotNull final RailsViewFoldersManager rootManager,
                                                      final boolean isRSpecSupportEnabled) {
