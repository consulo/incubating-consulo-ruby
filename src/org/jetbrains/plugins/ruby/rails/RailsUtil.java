...
public class RailsUtil {
    ...
    public static void generateRailsApp(final Module module,
        ...
        final RubyScriptRunnerArgumentsProvider provider =
                new RubyScriptRunnerArgumentsProvider(generatorArgsList.toArray(new String[generatorArgsList.size()]), null, null);
        final Filter[] filters = {new RFileLinksFilter(module, null), new GeneratorsLinksFilter(module)};
     ...
     */
    @Nullable
    public static String getPathRelativeToTailsApplicationRoot(@NotNull final String fileUrl,
    ...
    public static boolean isValidRailsFSPath(@NotNull final String string) {
        final StringTokenizer st = new StringTokenizer(string,
                String.valueOf(VirtualFileUtil.VFS_PATH_SEPARATOR));
        boolean isValid = string.length() > 0;
        while (st.hasMoreTokens() && isValid) {
            final String token = st.nextToken();
            isValid = NamingConventions.isInMixedCase(token)
                    || NamingConventions.isInUnderscoredCase(token);
        }
        return isValid;
    }
     ...
     */
    @Nullable
    public static PsiDirectory getPsiDirByRailsNode(@Nullable final RailsNode node,
     ...
     */
    @Nullable
    public static PsiFile getPsiFileByRailsNode(@Nullable final RailsNode node,
                                                @NotNull final Project project) {
        if (node == null) {
            return null;
        }
    ...
    @Nullable
    public static PsiElement getPsiElementByRailsNode(@Nullable final RailsNode node,
                                                      @NotNull final Project project) {
        if (node == null) {
            return null;
        }
    ...
    @Nullable
    public static PsiElement getPsiElementByNodeId(@NotNull final NodeId nodeId,
    ...
    public static String toModulesPath(@Nullable final String fsPath) {
        ...
        final StringTokenizer sTokenizer =
                new StringTokenizer(fsPath,
                        String.valueOf(VirtualFileUtil.VFS_PATH_SEPARATOR));
